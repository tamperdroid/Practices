...
config.read('D:/01_2017/17/Config.ini')
...
try:
    ...
    inifile = 'D:/01_2017/17/' + str(dsName) + '.ini'
	# print "ini file:",inifile
    config.read(config.read(inifile))
    Fundalytics_Extract = imp.load_source('Fundalytics_Extract','D:/01_2017/17/Fundalytics_Extract.py')
    extConfig = ''
    try:
		if '/B' in dsID:
			# print "backfilling:"
					...
					if int(NoOfDays) >= 31:
						...
						for dateTry in range(int(math.ceil(DateRange))):
							...
							if config.get('default', 'E_SCRAPY') != 'n/a':
								...
								print 'D:/01_2017/17/' + cutomFile + '.py'
								cutomFile = imp.load_source(cutomFile, 'D:/01_2017/17/' + str(cutomFile) + '.py')
								extConfig = cutomFile.main(config)
							else:
								extConfig = Fundalytics_Extract.main(config)

							Fundalytics_Transform = imp.load_source('Fundalytics_Transform','D:/01_2017/17/Fundalytics_Transform.py')
					...
					for dateTry in range(6):
						...
						if config.get('default', 'E_SCRAPY') != 'n/a':
							...
							cutomFile = imp.load_source(cutomFile,
														'D:/01_2017/17/' + str(cutomFile) + '.py')
							extConfig = cutomFile.main(config)
						else:
							extConfig = Fundalytics_Extract.main(config)

						Fundalytics_Transform = imp.load_source('Fundalytics_Transform',
																'D:/01_2017/17/Fundalytics_Transform.py')
						if (extConfig.get("status", "extractStatus") == "1"):
							trnsConfig = Fundalytics_Transform.main(extConfig)
						else:
							print "Exception to be raised"
