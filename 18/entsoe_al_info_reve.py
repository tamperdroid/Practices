        ...
        for eic1 in con2[1:]:
            for eic in con2[1:]:
                ...
                if ping1.status_code==200:
                    success_count+=1
                    with open("url_list_al_info_reve.txt","ab") as f:
                        f.write(str(success_count)+"\t"+str(formed_url))
                ping_count+=1
                with open("eic_codes_explicity_a_i_avnue_latest.txt","ab") as f:
                    f.write(str(ping_count)+"\t"+str(con_man_agree)+"\t"+str(eic)+"\t"+str(eic1)+"\t"+str(ping1.status_code)+"\n")
print "done"
