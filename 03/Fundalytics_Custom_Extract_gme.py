    ...
    try:
        ...
        for file_indx,file in enumerate(file_data):
            ...
            for i,data in enumerate(file):
                ...
                # print data[1]
                # val_interval=i%7
                # worksheet1.write(file_indx+row_inc,i,data[1])
                # print data[]
            ...
            for name in zf.namelist():
                ...
                if marketName=="mgp-transmission-limits":
                    ...
                    for LimitTransito in LimitTransito_list:
                        # print LimitTransito
                        data = re.findall("<(.*?)>(.*?)<\/.*?>", str(LimitTransito), re.I)
                ...
                if marketName == "mgp-transits":
                    data_list = re.findall(r"<MgpTransiti>([\w\W]*?)</MgpTransiti>", str(file_content),
                                                    re.I)
                    # print LimitTransito_list
                    # print "LimitTransito_list",len(LimitTransito_list)
                ...
                if marketName == "mgp-volumes":
                    data_list = re.findall(r"<Quantita>([\w\W]*?)</Quantita>", str(file_content),
                                           re.I)
                    # print LimitTransito_list
                    # print "LimitTransito_list",len(LimitTransito_list)
                    for datum in data_list:
