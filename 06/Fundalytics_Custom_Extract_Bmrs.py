...
import imp
import ftplib
#Fundalytics_Utility = imp.load_source('Fundalytics_Utility', 'D:/prabakaran/Integration/Fundalytics_Utility.py')
...
#tempFilePath = "D:/prabakaran/Integration/temp/"
tempFilePath = "D:/01_2017/06/temp/"
...
def navigation_function(DataSourceID, datasourceName, marketName, scraperParameterName, scraperType,
    ...
    try:
        ...
        if class_regex_url == 'CLASSNAME':
            ...
            print 'class_regex_url',class_regex_url
                    ...
                    try:
                        ...
                        try:
                            if 'GET' in split_nav[2]:
                                ...
                                raw_file = post_method(DataSourceID, datasourceName, marketName, scraperParameterName, scraperType,
...
def post_method(DataSourceID, datasourceName, marketName, scraperParameterName, scraperType, control, url, head,rawFile_time):
    ...
    try:
        ...
        if (control.get(scraperParameterName, 'E_NAVL1')) != 'n/a':
            ...
            urlRegex = splitNav[1].replace('REGEX{', '').replace('}', '')
            regexUrl = re.search(urlRegex, post_content, re.IGNORECASE)
            homeUrl = urlparse(url)
            getUrl = homeUrl[0] + '://' + homeUrl[1] + str(regexUrl.group(1))
            try:
                if 'GET' in splitNav[2]:
                    response = requests.get(getUrl, headers=header, cookies=con.cookies)
                    status = response.status_code
                    rawContent = response.content
                    rawFile = file_writing(DataSourceID,datasourceName, marketName, scraperParameterName, scraperType, control,rawContent, 'n/a',rawFile_time)
                elif 'POST' in splitNav[2]:
                    head = ast.literal_eval(splitNav[3])
                    rawFile = post_method(DataSourceID, datasourceName, marketName, scraperParameterName, scraperType,control, getUrl, head,rawFile_time)
...
def file_writing(DataSourceID,datasourceName, marketName, scraperParameterName, scraperType, control, raw_content, Ivalue,rawFile_time):
    try:
        ...
        if str(scraperType) in ('xls', 'xlsx'):
            ...
            workbook = workbook = open_workbook(rawFileName)
    ...
    try:
        ...
        if sessionNav1[0] == 'URL':
            ...
            if 'GET' in sessionNav1[2]:
                ...
                response_content = response.content
                raw_file = file_writing(DataSourceID, datasourceName, marketName, scraperParameterName, scraperType,
                                        control,
                                        response_content,
                                        'n/a',rawFile_time)
                ...
                for files in fileList:
                    if str(scraper_parameters) in files:
                        ...
                        if '.csv' in files:
                            ...
                            ws = wb.add_sheet(str(scraper_parameters))
                            
                            if str(scraper_parameters) in ['nouy1','nouy2','nouy3','nouy4','nouy5','zouy1','zouy2','zouy3','zouy4','zouy5']:
                                for rowx, row in enumerate(spamReader):
                                    try:
                                        if rowx>0 and row and row[3] and row[4]:
                                            week_row=row[3]
                                            year_row=row[4]
                                            d= str(year_row)+"-"+str(week_row)
                                            week_year_value = datetime.strptime(d + '-1', "%Y-%W-%w")
                                        else:
                                            week_year_value=''
                                    except:
                                        week_year_value=''
                                    ...
                                    for colx, value in enumerate(row):  
                                        if re.match('(\d{12,14})', value):
                                            if len(value)==12:
                                                value=value[0:4]+'-'+value[4:6]+'-'+value[6:8] +'T'+value[8:10]+':'+value[10:12]          
                                            else:
                                                value=value[0:4]+'-'+value[4:6]+'-'+value[6:8] +'T'+value[8:10]+':'+value[10:12]+':'+value[12:]                                           
                                        ws.write(rowx, colx, unicode(value, errors="ignore"))
                                    ws.write(rowx, 6,week_year_value)
                    ...
                    if marketName in ['wind-forecast-outturn','peak-demand','dayahead-margin','dayahead-demand','dayahead-demand-2to14']:
                        if '.csv' in files:          
                            deLimiter = control.get('default', 'E_DELIMITER')
                            spamReader = csv.reader(open(files, 'rb'), delimiter=(deLimiter.replace('VAL','').replace('{','').replace('}','')).strip())
                            ws = wb.add_sheet(str(scraper_parameters))                                        
...
def main(control):
    try:
        ...
        for scraper_parameters in spName:
            ...
            try:
                if str(marketName) in ['weekahead-data-2to52']:
                    #from datetime import datetime
                    ...
                    Year,WeekNum,DOW = current_week.isocalendar() # DOW = day of week
                    get_url_to_scrape=get_url_to_scrape_date.replace('{YYYY}',str(Year)).replace('{WW}',str(WeekNum))
                ...
                if method[0] in ("GET", 'GET_BY'):
                    ...
                    try:
                        if str(ck)!='{}':
                            ...
                            if str(marketName) in ['peak-demand','system-demand','system-price']:
                                content_type='csv'
                            '''downloading the extracted file and pushed to s3'''
                            rawFile = sessionfile(DataSourceID, datasourceName, marketName, scraperParameterName,scraperType, get_url_to_scrape, method[0], control, content_type, checked_response_content, response,dictID,s,rawFile_time)
                        ...
                        if str(marketName) in ['peak-demand','system-demand','system-price','weekahead-data-2to52']:
                                content_type='csv'                        
                        '''downloading the extracted file and pushed to s3'''
